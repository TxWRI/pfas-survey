...
included in the survey ([@tbl-q20]). The probability of
...
p2 <- marginal_m1$diffs |> 
  mutate(y = as_factor(str_replace(y, ",", ",\n"))) |> 
  ggplot() +
  geom_pointrange(aes(y = Q16, x = predicted,
                      xmin = conf.low, xmax = conf.high),
                  size = 0.25) +
  facet_wrap(~forcats::fct_rev(y), ncol = 1) +
...
cowplot::ggdraw(align_legend(p1+p2, hjust = 1))
...
([@tbl-model4]). For individuals identifying as other, the
...
There was a strong association between awareness of drinking water being contaminated
by PFAS and the level of concern about PFAS contamination in drinking water. There
was a 27% difference between individuals
that are and are not aware of PFAS contamination in their drinking water 
